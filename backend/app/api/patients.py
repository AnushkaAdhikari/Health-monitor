...
def update_patient(patient_id: int, payload: PatientCreate, db: Session = Depends(get_db), _=Depends(get_current_user)):
    patient = db.query(Patient).filter(Patient.id == patient_id).first()
    if not patient:
        raise HTTPException(status_code=404, detail="Patient not found")
    phone_owner = db.query(Patient).filter(Patient.phone == payload.phone, Patient.id != patient_id).first()
    if phone_owner:
        raise HTTPException(status_code=400, detail="A patient with this phone number already exists")
